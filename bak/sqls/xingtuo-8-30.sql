...
CREATE TABLE IF NOT EXISTS `xt_message` (
  ...
  PRIMARY KEY (`id`),
  KEY `user_id` (`user_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='消息表' AUTO_INCREMENT=4 ;
...
(1, 2, 2, 1, 'aaaaaaaaaaaaa', 0, 0, 0),
(2, 3, 2, 1, 'bbbbbbbbbbbbbbb', 0, 1, 0),
(3, 4, 2, 1, 'cccccccc', 0, 1, 0);
...
CREATE TABLE IF NOT EXISTS `xt_relation` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键',
  `director_id` int(10) unsigned NOT NULL COMMENT '客户经理ID',
  `user_id` int(10) unsigned NOT NULL COMMENT '客户ID',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='客户关系表' AUTO_INCREMENT=1 ;
...
(1, 'admin', '管理员', 'e10adc3949ba59abbe56e057f20f883e', 1377850076, '192.168.1.27', 108, 1376561926, 1376561926, 0, 0),
...
CREATE TABLE IF NOT EXISTS `xt_user_info` (
  ...
  PRIMARY KEY (`id`),
  KEY `user_id` (`user_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='用户详细资料' AUTO_INCREMENT=12 ;

--
-- 转存表中的数据 `xt_user_info`
--

INSERT INTO `xt_user_info` (`id`, `user_id`, `file_id`, `age`, `sex`, `email`, `province`, `city`, `street`, `interest`, `purchase`, `remarks`) VALUES
(1, 1, 3, 21, '男', 'ucdchina@gmail.com', '广东省', '深圳市', '111号', '理财产品', '无', '这是管理员aaeqweqe'),
(2, 2, 4, 21, '男', 'lin@qq.com', '上海', '上海市', '陆家嘴', '期货产品', '无', NULL),
(3, 3, 0, 22, '女', 'wuzhiyong', '上海', '上海市', '陆家嘴', '暂无', '无', NULL),
(5, 5, 0, 7, '男', 'aaaaa@qq.com', 'bbbb', 'ccc', 'ddde', 'eee', 'fff', 'ggggggggg~'),
(6, 100, 0, 6, '女', '', '', '', '', '', '', ''),
(10, 6, 0, 6, '女', '12313123@qq.com', '12312312bb', '22222222', '', '', '', ''),
(11, 7, 0, 5, '男', 'zzz@qq.com', '13123', '2423', '423423', '324', '234234', '');
